...
~/EnsembleVariantCallingPipeline/postalign_check_template.sh $sampleF $out 
...
do
	...
	mkdir -p ${out}/${sample}
	if [ ! -f "${out}/${sample}/${sample}_tumor_final.bam" ]; then
		ln -s ${path}/${tumor}.bam ${out}/${sample}/${sample}_tumor_final.bam || echo ${tumor}.bam not found
		ln -s ${path}/${tumor}.bam.bai ${out}/${sample}/${sample}_tumor_final.bam.bai || echo ${tumor}.bam.bai not found
	fi
	
	if [ ! -f "${out}/${sample}/${sample}_normal_final.bam" ]; then
		ln -s ${path}/${normal}.bam ${out}/${sample}/${sample}_normal_final.bam || echo ${normal}.bam not found
		ln -s ${path}/${normal}.bam.bai ${out}/${sample}/${sample}_normal_final.bam.bai || echo ${normal}.bam.bai not found
	fi

	~/EnsembleVariantCallingPipeline/pon_template.sh $email $sample $ref $out
	~/EnsembleVariantCallingPipeline/strelka_template.sh $email $sample $ref $out $type
	~/EnsembleVariantCallingPipeline/varscan_template.sh $email $sample $ref $out $purity
	~/EnsembleVariantCallingPipeline/mutect_template.sh $email $sample $ref $out $pon $type $dbSNP ${interval_list}
	~/EnsembleVariantCallingPipeline/muse_template.sh $email $sample $ref $out $type $dbSNP
	~/EnsembleVariantCallingPipeline/postAlignment_template.sh $email $sample $ref $out ${known_indel_list} ${base_recalibration_list}
done